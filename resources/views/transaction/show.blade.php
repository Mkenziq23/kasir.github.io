                                        ...
                                        <div class="row  justify-content-between mb-3 pe-2">
                                            <div class="col-auto col-md-7">
                                                <div class="media flex-column flex-sm-row">
                                                    <img class=" img-fluid"
                                                        src="{{ asset('storage/' . $item->menu->picture) }}" width="62"
                                                        height="62">
                                                    <div class="media-body  my-auto">
                                                        <div class="row ">
                                                            <div class="col-auto">
                                                                <p class="mb-0"><b>{{ $item->menu->name }}</b></p><small
                                                                    class="text-muted">{{ $item->menu->category }}</small>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                ...
                                <div class="row">
                                    <div class="col">
                                        ...
                                        <div class="row justify-content-between mb-2">
                                            ...
                                            <div class="flex-sm-col col-auto">
                                                <p class="mb-1"><b>IDR
                                                        {{ number_format($key->total_transaction, 0, ',', '.') }}</b></p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            @if ($key->status == 'paid')
                                <div class="card-body">
                                    ...
                                    <div class="form-group mb-3">
                                        <label for="name_cashier" class="small text-muted fw-semibold mb-1">NAME ON
                                            CASHIER</label>
                                        <input type="text" class="form-control form-control-sm" disabled
                                            name="name_cashier" id="name_cashier" value="{{ $key->user->name }}">
                                    </div>
                                    {{-- <div class="form-group mb-3">
                                        <label for="no_table" class="small text-muted fw-semibold mb-1">NO TABLE</label>
                                        <input type="text" class="form-control form-control-sm" disabled name="no_table"
                                            id="no_table" value="{{ $key->no_table }}">
                                    </div> --}}
                                    <div class="form-group mb-3">
                                        <label for="total_transaction" class="small text-muted fw-semibold mb-1">TOTAL
                                            TRANSACTION</label>
                                        <input type="text" class="form-control form-control-sm" disabled
                                            name="total_transaction" id="total_transaction"
                                            value="{{ number_format($key->total_transaction, 0, ',', '.') }}">
                                    </div>
                                    <div class="form-group mb-3">
                                        <label for="total_payment" class="small text-muted fw-semibold mb-1">TOTAL
                                            PAYMENT</label>
                                        <input type="text" class="form-control form-control-sm" name="total_payment"
                                            value="{{ number_format($key->total_payment, 0, ',', '.') }}" id="price"
                                            disabled>
                                    </div>
                                    <div class="row mb-md-5 mt-4">
                                        <div class="col">
                                            <a onclick="show_my_receipt1()"
                                                class="text-white btn btn-info w-100 btn-block ">Show Receipt</a>
                                        </div>
                                    </div>
                                </div>
                            @else
                                <form action="/transaction/{{ $key->id }}" method="POST" class="card-body">
                                    ...
                                    <div class="form-group mb-3">
                                        ...
                                        <input type="text" class="form-control form-control-sm" disabled
                                            name="total_transaction" id="total_transaction"
                                            value="{{ number_format($key->total_transaction, 0, ',', '.') }}">
                                        <input type="hidden" id="total" name="total_transaction"
                                            value="{{ $key->total_transaction }}">
                                    </div>
                                    <div class="form-group mb-3">
                                        <label for="total_payment" class="small text-muted fw-semibold mb-1">TOTAL
                                            PAYMENT</label>
                                        <input type="text"
                                            class="form-control form-control-sm total_payment @error('total_payment') is-invalid @enderror"
                                            id="price">
                                    ...
                                    <div class="row mb-md-5 mt-4">
                                        <div class="col">
                                            <button type="submit" onclick="show_my_receipt2()"
                                                class="text-white btn btn-primary w-100 btn-block ">PAY
                                                {{ number_format($key->total_transaction, 0, ',', '.') }}</button>
                                        </div>
                                    </div>
                                </form>
                            @endif
